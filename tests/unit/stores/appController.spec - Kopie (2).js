...
describe('test "stores/appController"', () => {
  ...
  test('whether progressTypingCheck plays correct key sound audio when enabled', async () => {
    ...
    // ✅ Intercept every construction of "new Audio(...)"
    window.Audio = new Proxy(AudioMock, {
      construct(target, args) {
        return new target(...args);
      }
    });

    const spyConstructor = vi.spyOn(window, 'Audio');
    ...
    expect(spyConstructor).toHaveBeenNthCalledWith(1, './good.mp3');
    expect(spyConstructor).toHaveBeenNthCalledWith(2, './good.mp3');
    expect(spyConstructor).toHaveBeenNthCalledWith(3, './bad.mp3');
    ...
    expect(spyConstructor).toHaveBeenNthCalledWith(4, './good.mp3');
    expect(spyConstructor).toHaveBeenNthCalledWith(5, './bad.mp3');
    expect(spyConstructor).toHaveBeenNthCalledWith(6, './good.mp3');
